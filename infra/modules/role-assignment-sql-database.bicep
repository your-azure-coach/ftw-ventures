...
module sqlServerRoleAssignement 'role-assignment-sql-server.bicep' = {
  name: 'ra-sqls-${take(guid(sqlServerName, sqlDatabaseName, principalAppId), 42)}-${deploymentId}'
  params: {
...
module sqlDatabaseRoleAssignment 'deployment-script.bicep' = {
  name: 'ra-sqldb-${take(guid(sqlDatabaseName, principalAppId), 41)}-${deploymentId}'
  scope: az.resourceGroup(scriptResourceGroupName)
  params: {
    name: 'deploy-sqldb-role-assignment-${guid(sqlServerName, sqlDatabaseName, principalAppId, deploymentId)}'
    scriptContent: loadTextContent('scripts/sql-database-role-assignment.ps1')
    scriptArguments: '-SqlResourceGroupName ${resourceGroup().name} -SqlServerName ${sqlServerName} -SqlServerFqdn ${sqlServer.properties.fullyQualifiedDomainName} -SqlDatabaseName ${sqlDatabaseName} -PrincipalName ${principalName} -PrincipalAppId ${principalAppId} -DatabaseRoles ${sqlDatabaseRoles}'
